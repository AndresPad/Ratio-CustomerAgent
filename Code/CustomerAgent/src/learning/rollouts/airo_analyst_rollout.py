"""AGL rollout wrapper for the AIRO analyst agent.
...
def _get_client() -> tuple[AzureOpenAI, str]:
    ...
    if _client is None:
        ...
        logger.info("AIRO analyst rollout client: endpoint=%s, model=%s", endpoint, _model)
    return _client, _model


@agl.rollout
def airo_analyst_rollout(
    task: dict,
    prompt_template: agl.PromptTemplate,
) -> float:
    """Execute the AIRO analyst on a metrics query and return a reward.
    ...
    try:
        ...
        user_text = (
            f"Analyze the following AIRO metrics query and provide the SQL and analysis.\n\n"
            f"Query: {task['input']}"
        )
        ...
        logger.info("AIRO analyst rollout reward: %.3f", reward)
        return reward

    except Exception as e:
        logger.error("AIRO analyst rollout failed: %s", e)
        return 0.0
