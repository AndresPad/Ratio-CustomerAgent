"""AGL rollout wrapper for the customer insights agent.

Uses a synchronous AzureOpenAI client directly (not the MAF agent factory).
Reuses compute_analyst_reward from F11.
"""
...
from learning.rewards import compute_analyst_reward
...
def _get_client() -> tuple[AzureOpenAI, str]:
    ...
    if _client is None:
        ...
        logger.info("Customer insights rollout client: endpoint=%s, model=%s", endpoint, _model)
    return _client, _model


@agl.rollout
def customer_insights_rollout(
    task: dict,
    prompt_template: agl.PromptTemplate,
) -> float:
    """Execute the customer insights agent on a query and return a reward.

    Fully synchronous — no async, no threads.

    Args:
        task: Dict with 'input', 'expected_sql', 'expected_output' keys.
    ...
    try:
        ...
        user_text = (
            f"Analyze the following customer impact query and provide the SQL and analysis.\n\n"
            f"Query: {task['input']}"
        )
        ...
        logger.info("Customer insights rollout reward: %.3f", reward)
        return reward

    except Exception as e:
        logger.error("Customer insights rollout failed: %s", e)
        return 0.0
