"""AGL rollout wrapper for the outage analyst agent.
...
def _get_client() -> tuple[AzureOpenAI, str]:
    ...
    if _client is None:
        ...
        logger.info("Outage analyst rollout client: endpoint=%s, model=%s", endpoint, _model)
    return _client, _model


@agl.rollout
def outage_analyst_rollout(
    task: dict,
    prompt_template: agl.PromptTemplate,
) -> float:
    """Execute the outage analyst on an incident query and return a reward.
    ...
    try:
        ...
        user_text = (
            f"Analyze the following outage/incident query and provide the SQL and analysis.\n\n"
            f"Query: {task['input']}"
        )
        ...
        logger.info("Outage analyst rollout reward: %.3f", reward)
        return reward

    except Exception as e:
        logger.error("Outage analyst rollout failed: %s", e)
        return 0.0
