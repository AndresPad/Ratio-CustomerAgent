/**
 * ChaNeuralCanvasV2Page — "Investigation Reasoning Flow" (Neural Canvas v2)
 *
 * Second-generation Neural Canvas mounted alongside the original
...
/**
 * Map the backend's `useLiveInvestigation` stage taxonomy onto the
 * Neural Canvas's 7-stage narrative. Live events flow as:
 *   signal → evaluation → hypothesis → scoring → selection
 *         → tool_execution → summary
 * The canvas visualises:
 *   signal → symptom → hypothesis → evidence → scoring → reasoning → result
 */
const LIVE_TO_CANVAS: Record<LiveStage, Stage> = {
  signal: 'signal',
  evaluation: 'symptom',
...
/** Convert the live hook's `stagesReached` into canvas reached stages,
 *  filling intermediate canvas stages so the progression looks continuous. */
function mapLiveReached(liveReached: readonly LiveStage[]): Stage[] {
  if (liveReached.length === 0) return [];
  const canvasTargets = new Set<Stage>(liveReached.map((s) => LIVE_TO_CANVAS[s]));
  // Always include 'evidence' once we've hit tool_execution, since live
  // collapses evidence/reasoning into one stage but the canvas shows both.
  if (canvasTargets.has('reasoning')) canvasTargets.add('evidence');
  const result: Stage[] = [];
  for (const s of STAGES) {
    if (canvasTargets.has(s)) result.push(s);
  }
  return result;
}
...
function useFlow(enabled: boolean = true) {
...
function StatStrip({
  ...
  reached: Stage[];
  counts?: { symptoms?: number; hypotheses?: number; evidence?: number; confidence?: number };
}) {
  const has = (s: Stage) => reached.includes(s);
  const fmt = (v: number | undefined, fallback: string): string =>
    v === undefined ? fallback : String(v);
  const stats = [
    { icon: 'fa-bolt', label: 'Signal', value: has('signal') ? '1' : '—', color: STAGE_META.signal.color },
    { icon: 'fa-exclamation-triangle', label: 'Symptoms', value: has('symptom') ? fmt(counts?.symptoms, '3') : '—', color: STAGE_META.symptom.color },
    { icon: 'fa-lightbulb', label: 'Hypotheses', value: has('hypothesis') ? fmt(counts?.hypotheses, '3') : '—', color: STAGE_META.hypothesis.color },
    { icon: 'fa-search', label: 'Evidence', value: has('evidence') ? fmt(counts?.evidence, '8') : '—', color: STAGE_META.evidence.color },
    { icon: 'fa-check-circle', label: 'Confidence', value: has('scoring') ? (counts?.confidence !== undefined ? `${counts.confidence}%` : '92%') : '—', color: STAGE_META.result.color },
  ];
  return (
    <div style={STATS_ROW}>
      {stats.map(s => (
...
function ScoringPanel({ visible, confidence }: { visible: boolean; confidence?: ConfidenceScore[] }) {
  if (!visible) return null;
  const rows = confidence ?? CONFIDENCE;
  return (
    <div style={{ ...CARD, borderLeft: `3px solid ${STAGE_META.scoring.color}`, animation: 'cha-fade-in .35s ease both' }}>
      ...
      {rows.length === 0 && (
        <div style={{ fontSize: 11, color: '#8a8faa' }}>Awaiting scored hypotheses…</div>
      )}
      {rows.map(c => {
...
function ReasoningPanel({
  ...
  visible: boolean;
  lines?: { text: string; type: TraceLine['type']; indent?: boolean }[];
}) {
  if (!visible) return null;
  const lines = liveLines ?? TRACE.filter(l => l.stage === 'scoring' || l.stage === 'reasoning');
  return (
...
function RelationshipGraph({ visible, symptoms }: { visible: boolean; symptoms?: Symptom[] }) {
  if (!visible) return null;
  const rows = symptoms ?? SIGNAL.symptoms;
  return (
...
function ResultPanel({
  ...
  const score = confidenceScore ?? 92;
  const duration = durationSeconds ?? 12.6;
  const evCount = evidenceCount ?? 8;
  const factorList = factors ?? ['Missing Index', 'Deploy #4821', '2.1M Row Scan'];
  return (
    <div style={{ ...CARD, borderLeft: `3px solid ${STAGE_META.result.color}`, background: '#f0fdf4', animation: 'cha-slide-in .4s ease both' }}>
      ...
      <p style={{ fontSize: 13, color: '#374151', lineHeight: 1.7, margin: '0 0 14px' }}>{rc.description}</p>
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {factorList.map((f, i) => (
            <span key={i} style={FACTOR_TAG}><i className="fas fa-tag" style={{ marginRight: 4 }} />{f}</span>
          ))}
        </div>
      </div>
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ fontSize: 32, fontWeight: 800, color: STAGE_META.result.color }}>{score}%</div>
          <div style={{ flex: 1 }}>
            <div style={{ height: 8, borderRadius: 4, background: '#dcfce7', overflow: 'hidden' }}>
              <div style={{ height: '100%', borderRadius: 4, width: `${score}%`, background: STAGE_META.result.color, transition: 'width 1.2s ease' }} />
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ marginTop: 14, display: 'flex', gap: 20 }}>
        <div style={{ fontSize: 11, color: '#5c6370' }}><i className="fas fa-clock" style={{ marginRight: 4 }} />Duration: {duration.toFixed(1)}s</div>
        <div style={{ fontSize: 11, color: '#5c6370' }}><i className="fas fa-exchange-alt" style={{ marginRight: 4 }} />7 stages</div>
        <div style={{ fontSize: 11, color: '#5c6370' }}><i className="fas fa-search" style={{ marginRight: 4 }} />{evCount} evidence items</div>
      </div>
    </div>
  );
}
...
/* ═══════════════════════════════════════════════════════════════ */
/*  Live → Canvas data adapter                                     */
/* ═══════════════════════════════════════════════════════════════ */

interface CanvasLiveView {
  signalTitle: string;
  symptoms: Symptom[];
  confidence: ConfidenceScore[];
  rootCause: RootCause | null;
  reasoningLines: { text: string; type: TraceLine['type']; indent?: boolean }[];
  counts: { symptoms: number; hypotheses: number; evidence: number; confidence?: number };
  durationSeconds?: number;
  factors: string[];
}

/** Shorten a long string for single-line display. */
...
/** Build the canvas view-model from `useLiveInvestigation` state. */
function buildLiveCanvasView(
  state: ReturnType<typeof useLiveInvestigation>['state'],
): CanvasLiveView {
  const { hypotheses, toolCalls, agentTurns, summary, selectedHypothesisId, investigationId } = state;

  // Distribute tool calls round-robin across hypotheses as evidence.
  const ev: EvidenceItem[][] = hypotheses.map(() => []);
  if (hypotheses.length > 0) {
    toolCalls.forEach((tc, i) => {
      const bucket = i % hypotheses.length;
      const status: EvidenceItem['status'] = tc.error
        ? 'failure'
        : tc.row_count > 0
          ? 'success'
          : 'neutral';
      ev[bucket].push({
        title: tc.tool || 'tool',
        detail: tc.error
          ? `error: ${truncate(tc.error, 90)}`
          : `${tc.row_count} row${tc.row_count === 1 ? '' : 's'} · ${tc.duration_ms} ms${tc.agent ? ` · ${tc.agent}` : ''}`,
        status,
      });
    });
  }

  const symptoms: Symptom[] = hypotheses.map((h, i) => ({
    title: truncate(h.matched_symptoms || h.statement, 100) || `Hypothesis ${h.rank || i + 1}`,
    hypothesis: {
      id: h.hypothesis_id || `HYP-${i + 1}`,
      label: truncate(h.statement, 140),
      prior: Math.round((h.match_score || 0) * 100),
      confidence: Math.round((h.confidence || 0) * 100),
    },
    evidence: ev[i] || [],
  }));

  const confidence: ConfidenceScore[] = hypotheses.map((h, i) => ({
    id: h.hypothesis_id || `HYP-${i + 1}`,
    label: truncate(h.statement, 100),
    score: Math.round((h.confidence || 0) * 100),
  }));

  // Root cause — from selected hypothesis + latest action_planner / reasoner turn.
  const selected =
    hypotheses.find((h) => h.hypothesis_id === selectedHypothesisId) ||
    hypotheses.find((h) => h.selected) ||
    (hypotheses.length ? [...hypotheses].sort((a, b) => (b.confidence || 0) - (a.confidence || 0))[0] : null);
  const actionTurn = [...agentTurns]
    .reverse()
    .find((t) => /action_planner|reasoner/i.test(t.agent));
  const rootCause: RootCause | null = selected
    ? {
        description: truncate(selected.statement, 600) || 'Investigation complete.',
        recommendedAction: actionTurn
          ? truncate(actionTurn.text, 600)
          : 'Review collected evidence for remediation steps.',
      }
    : null;

  // Reasoning trace — last agent turns + last tool calls.
  const reasoningLines: CanvasLiveView['reasoningLines'] = [];
  agentTurns.slice(-6).forEach((t) => {
    reasoningLines.push({
      text: `[${t.agent}] ${truncate(t.text, 200)}`,
      type: 'normal',
    });
  });
  toolCalls.slice(-5).forEach((tc) => {
    reasoningLines.push({
      text: `→ ${tc.tool} (${tc.row_count} rows, ${tc.duration_ms}ms)`,
      type: tc.error ? 'fail' : 'success',
      indent: true,
    });
  });
  if (selected) {
    reasoningLines.push({
      text: `Leading hypothesis: ${selected.hypothesis_id} at ${Math.round((selected.confidence || 0) * 100)}%`,
      type: 'highlight',
    });
  }

  const topScore = confidence.length > 0 ? Math.max(...confidence.map((c) => c.score)) : undefined;

  // Contributing factors — phases traversed.
  const factors = state.phaseHistory.length > 0 ? state.phaseHistory.slice(-6) : [];

  return {
    signalTitle: investigationId ? `Investigation ${investigationId}` : (state.customer_name || 'Live Investigation'),
    symptoms,
    confidence,
    rootCause,
    reasoningLines,
...
export default function ChaNeuralCanvasV2Page() {
  ...
  // Mock flow only auto-plays when mock mode is selected.
  const mockFlow = useFlow(mode === 'mock');
  const live = useLiveInvestigation();

  const isLive = mode !== 'mock';

  const stage: Stage | null = isLive
    ? (live.state.stagesReached.length > 0 ? LIVE_TO_CANVAS[live.state.stage] : null)
    : mockFlow.stage;
  const reached: Stage[] = isLive
    ? mapLiveReached(live.state.stagesReached)
    : mockFlow.reached;
  const running = isLive ? live.state.running : mockFlow.running;
  const elapsed = isLive ? 0 : mockFlow.elapsed;
  const traceCount = isLive ? live.state.events.length : mockFlow.traceCount;
  const liveError = isLive ? live.state.error : null;

  // Live view-model (only computed when isLive).
  const liveView = useMemo(
    () => (isLive ? buildLiveCanvasView(live.state) : null),
    [isLive, live.state],
  );
  ...
  const handleStart = useCallback(() => {
    ...
    if (mode === 'replay' && !xcv.trim()) {
      return;
    }
    live.start({
      mode,
      xcv: xcv.trim() || undefined,
      agentFilter: agentFilter.trim() || undefined,
      pollPacingMs: mode === 'replay' ? pollPacingMs : undefined,
    });
  }, [mode, xcv, agentFilter, pollPacingMs, mockFlow, live]);

  const handleStop = useCallback(() => {
    if (isLive) live.stop();
  }, [isLive, live]);

  // Stop live stream when switching modes.
  useEffect(() => {
    if (mode === 'mock') live.stop();
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [mode]);

  const has = (s: Stage) => reached.includes(s);
  const startDisabled = running || (mode === 'replay' && !xcv.trim());

  // Track elapsed time while waiting for the first event so live mode
  // doesn't look silently stuck (cold-start can take 10–30s).
  const [waitElapsedMs, setWaitElapsedMs] = useState(0);
  const waitingForFirst = isLive && running && traceCount === 0;
  useEffect(() => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#f5f6fa', overflow: 'hidden' }}>
      ...
      {/* Stats strip */}
      <StatStrip reached={reached} counts={liveView?.counts} />

      {/* Activity bar (agent + phase + elapsed) */}
      <ActivityBar stage={stage} running={running} elapsed={elapsed} />

      {/* Source toggle + Run / Stop bar */}
      <div
        style={{
          display: 'flex', alignItems: 'center', justifyContent: 'space-between',
          gap: 12, padding: '8px 20px 0', flexShrink: 0, flexWrap: 'wrap',
        }}
      >
        <DataSourceToggle
          mode={mode}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          {liveError && (
            <span style={{ fontSize: 11, color: '#d1242f', fontWeight: 600 }} title={liveError}>
              <i className="fas fa-exclamation-triangle" /> {liveError.length > 60 ? liveError.slice(0, 60) + '…' : liveError}
            </span>
          )}
          <span style={{ fontSize: 11, color: '#5c6370', fontWeight: 600 }}>
            {traceCount} {isLive ? 'events' : 'lines'}
          </span>
          {running && isLive && (
            <button
              onClick={handleStop}
              className="cha-btn-secondary"
              style={{
                display: 'flex', alignItems: 'center', gap: 6,
                padding: '6px 14px', fontSize: 12, fontWeight: 600, borderRadius: 6,
                border: '1px solid #d1242f', cursor: 'pointer',
                background: '#fff', color: '#d1242f',
              }}
            >
          ...
          <button
            ...
            style={{
              ...
              border: 'none', cursor: startDisabled ? 'not-allowed' : 'pointer',
              background: startDisabled ? '#adb5bd' : '#4f6bed', color: '#fff',
              transition: 'background .15s',
            }}
            title={mode === 'replay' && !xcv.trim() ? 'Enter an xcv to replay' : undefined}
          >
            <i className={`fas ${running ? 'fa-spinner fa-spin' : mode === 'mock' ? 'fa-redo' : 'fa-play'}`} />
            {running ? 'Running…' : mode === 'mock' ? 'Mock' : mode === 'replay' ? 'Polling' : 'New Run'}
          </button>
        </div>
      </div>

      {/* Polling stopwatch (only when polling) */}
      {mode === 'replay' && (
        <div style={{ padding: '8px 20px 0' }}>
          <PollingStopwatch
            running={running}
            eventCount={traceCount}
      ...
      {waitingForFirst && (
        <div style={{
          margin: '8px 20px 0', padding: '8px 12px', background: '#f4f8ff', color: '#1a2a6b',
          border: '1px solid #b6ccff', borderRadius: 8, fontSize: 12,
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px 20px' }}>
            ...
            <div style={{ ...CARD, borderLeft: `3px solid ${STAGE_META.signal.color}`, animation: 'cha-fade-in .3s ease both' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <span style={{ fontWeight: 700, fontSize: 15, color: '#1a1a2e' }}>
                  {liveView ? liveView.signalTitle : SIGNAL.title}
                </span>
              </div>
              {isLive && live.state.signalDecision && (
                <div style={{ fontSize: 11, color: '#5c6370', marginTop: 6 }}>
                  {live.state.signalDecision.action} · {live.state.signalDecision.signal_count} signals · {live.state.signalDecision.compound_count} compounds
                </div>
              )}
            </div>
          )}
          ...
          {/* Symptom cards */}
          {has('symptom') && (liveView ? liveView.symptoms : SIGNAL.symptoms).map((sym, i) => (
            <SymptomCard
              key={i}
              sym={sym}
          ...
          <RelationshipGraph visible={has('evidence')} symptoms={liveView?.symptoms} />

          {/* Confidence scoring */}
          <ScoringPanel visible={has('scoring')} confidence={liveView?.confidence} />

          {/* Reasoning */}
          <ReasoningPanel visible={has('reasoning')} lines={liveView?.reasoningLines} />
          ...
          <ResultPanel
            visible={has('result')}
            rootCause={liveView?.rootCause ?? undefined}
            confidenceScore={liveView?.counts.confidence}
            durationSeconds={liveView?.durationSeconds}
            evidenceCount={liveView?.counts.evidence}
            factors={liveView?.factors}
          />
        </div>
